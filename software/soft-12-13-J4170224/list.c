...
Node *push_front(Node *begin, const char *str)
{
  ...
  strcpy(s, str); // 文字列をコピーする
  p->str = s; //確保した構造体メンバーの文字列（=charポインタ）に代入
  p->next = begin; //引数で受け取った構造体を次の行き先に設定
...
Node *push_back(Node *begin, const char *str)
{
  ...
  strcpy(s, str);
  q->str = s;
  q->next = NULL; //今回確保したものは末尾にあるので、nextがNULLをさすようにする
...
//
// 出力:
// - 先頭になるNodeへのポインタ（begin と同じ）
//
Node *pop_back(Node *begin)
{
  // 終端を削除する場合は、一つ前を後から終端に指定 (p->next =NULL) する必要がある。

  // 先頭がNULLの場合は何もする必要がない
  if (begin == NULL){
    return begin;
  }

  // 以下はそれ以外の場合
  // 二箇所 (現在p と次q) を覚えておく必要がある
  // 次q の次 (q->next) がNULL かどうかが終了判定
  Node *p = begin;
  Node *q = p->next;

  // この時点でq==NULL (要素が一つだけだった場合) は p を削除して終わり
  if (q == NULL){
    free(p->str);
    free(p);
    return q;
  }
  // q->next がNULLまでポインタを順次たどる
  while (q->next != NULL){
    p = q;
    q = p->next;
  }
